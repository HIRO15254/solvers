...
Source hashes and exact numeric records are in [the companion JSON](result.json). Raw inputs are the saved `audit.stdout.json`, `config.toml`, and execution/progress records under the run directories identified in that JSON.
...
[最新方針・結果・未完了項目](../scaling-note.md)を参照。
