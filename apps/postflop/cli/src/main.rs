//! `postflop-solver`: the postflop app's CLI (Mode A exact postflop).
//!
//! A thin `clap` binary over the shared `app-core` library. Accepts
//! `kind = "postflop"` configs (plus Kuhn/Leduc as engine smoke checks);
//! preflop and multiway configs are redirected to `preflop-solver`. The
//! postflop web UI (and its bridge `serve` command) is a roadmap item —
//! see `docs/app-structure.md` for the app-level design.

use anyhow::Result;
use app_core::{App, bench, ensure_config_kind, inspect, report, resume, sol, solve};
use clap::{Parser, Subcommand};

#[derive(Parser)]
#[command(
    name = "postflop-solver",
    about = "Postflop solver (exact, fixed flop)",
    version
)]
struct Cli {
    #[command(subcommand)]
...
enum Command {
    ...
    Solve {
        /// Path to the config file (see examples/postflop_srp20.toml).
        ...
        /// (browse it later with `inspect --sol`).
        #[arg(long)]
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Command::Solve {
            ...
            ensure_config_kind(&config, App::Postflop)?;
            solve::run(
                &config,
                output.as_deref(),
                &history,
