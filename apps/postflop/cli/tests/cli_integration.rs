...
fn workspace_root() -> std::path::PathBuf {
    // apps/postflop/cli/tests -> up to workspace root
...
fn inspect_smoke() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_postflop-solver"))
        .arg("inspect")
        .arg(&config)
        .env("NO_COLOR", "1")
        .stdin(Stdio::piped())
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn postflop-solver inspect");
    child
        .stdin
        .take()
        .unwrap()
    ...
    let output = child
        .wait_with_output()
        .expect("wait for postflop-solver inspect");
    assert!(
...
fn report_smoke() {
    let config = workspace_root().join("examples/river_small.toml");
    let output = Command::new(env!("CARGO_BIN_EXE_postflop-solver"))
        .arg("report")
        .arg(&config)
        .arg("--boards")
        .arg("2c 7d 9h Js Qs,2c 7d 9h Js Ks")
        .output()
        .expect("run postflop-solver report");
    assert!(
        output.status.success(),
        "stderr: {}",
        String::from_utf8_lossy(&output.stderr)
...
fn run_solver(args: &[&str]) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_postflop-solver"))
        .args(args)
        .output()
        .expect("run postflop-solver")
}
...
#[test]
fn preflop_config_is_redirected_to_the_preflop_app() {
    let config = workspace_root().join("examples/pushfold_10bb.toml");
    let output = run_solver(&["solve", config.to_str().unwrap()]);
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("preflop-solver"),
        "expected a pointer to the preflop app, got: {stderr}"
    );
}
...
/// `crates/app-core/src/sol.rs`'s own unit-test fixture, duplicated here
/// (rather than shared) since this is a separate test binary with no access
/// to `app-core`'s internal `sol` module fixtures.
...
fn sol_export_and_inspect_smoke() {
    ...
    let mut child = std::process::Command::new(env!("CARGO_BIN_EXE_postflop-solver"))
        .arg("inspect")
        .arg("--sol")
        .arg(&sol_path)
        .env("NO_COLOR", "1")
        ...
        .spawn()
        .expect("spawn postflop-solver inspect --sol");
    child
        .stdin
        .take()
        .unwrap()
