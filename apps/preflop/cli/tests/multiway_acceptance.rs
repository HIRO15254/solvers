...
fn workspace_root() -> PathBuf {
    ...
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("cli directory has an app parent")
        .parent()
        .expect("app directory has an apps parent")
        .parent()
        .expect("apps directory has a workspace parent")
        .to_path_buf()
}
...
fn two_player_push_fold_tracks_the_existing_hu_exact_solver() {
    ...
    let hu_process = Command::new(env!("CARGO_BIN_EXE_preflop-solver"))
        .arg("solve")
        .arg(&hu_config_path)
        .arg("--output")
        .arg(&hu_output_path)
    ...
    let multiway_process = Command::new(env!("CARGO_BIN_EXE_preflop-solver"))
        .arg("solve")
        .arg(&multiway_config)
        .arg("--output")
