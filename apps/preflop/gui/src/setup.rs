...
pub struct SetupState {
    ...
    /// Background `app_core::auto_run::derive_auto_run` preview for Auto mode's
...
fn run_section(ui: &mut Ui, state: &mut SetupState) {
    ui.collapsing("Run", |ui| {
        ...
        ui.horizontal(|ui| {
            ui.label("storage:");
            ui.selectable_value(&mut run.storage, app_core::config::StorageKind::F32, "f32");
            ui.selectable_value(&mut run.storage, app_core::config::StorageKind::I16, "i16");
        });
