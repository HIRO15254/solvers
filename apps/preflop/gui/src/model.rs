//! Editable mirror of `app_core::config::SolveConfig` for `kind =
...
use app_core::session::PRUNE_THRESHOLD_STAKE_FACTOR;
use multiway::SeatId;
use multiway::config::{
...
pub struct AlgorithmModel {
    ...
    /// `app_core::session::build_multiway_session`'s derivation, mirrored by
...
/// (see `app_core::session::convert_rake`), while the GUI edits plain bb values.
...
pub fn model_to_solve_config(model: &Model) -> Result<SolveConfig, String> {
    ...
    Ok(SolveConfig {
        ...
        algorithm: AlgorithmSection::ExternalSamplingMccfr {
            ...
            // default (see `app_core::config::default_prune_skip_probability`),
...
/// `app_core::session::build_multiway_session` does when `algorithm.prune = true`
...
/// tournament ICM. See `app_core::session::derive_prune_threshold`'s doc comment
...
/// testable.
///
/// Derives (via `app_core::auto_run::derive_auto_run`) `run.sweep_batch` and a
...
pub fn apply_auto_derivation(
    ...
    let derivation = app_core::auto_run::derive_auto_run(game_config, threads, memory_budget_bytes)
...
/// checks `app_core::session::build_multiway_session` performs before it trains
...
pub fn validate(model: &Model) -> Vec<String> {
    ...
    match app_core::session::convert_utility(config.utility) {
        Ok(utility) => {
            let rake = app_core::session::convert_rake(config.rake);
    ...
    // (`app_core::session::build_multiway_session`) still hard-rejects the
...
mod tests {
    ...
    fn built_in_presets_round_trip_model_to_toml_and_validate() {
        for &(name, toml_text) in presets::BUILT_IN {
            ...
            let utility = app_core::session::convert_utility(reparsed_config.utility)
                .unwrap_or_else(|error| panic!("preset {name} utility conversion failed: {error}"));
            let rake = app_core::session::convert_rake(reparsed_config.rake);
            game_config
...
"#;
    ...
    fn apply_auto_derivation_lands_every_derived_field_and_writes_them_explicitly() {
        ...
        // `app_core::auto_run::derive_auto_run`) cheap even at full default
        // betting sizes -- the same scale `app_core::auto_run`'s own tests use.
