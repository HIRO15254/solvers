...
/// `run.max_memory_bytes = None` mirror of `app_core::session::DEFAULT_MEMORY_LIMIT`
...
impl EstimatePanelState {
    ...
    fn dispatch(&mut self, model: &Model, hash: u64) {
        ...
        let app_core::config::GameSection::PreflopMultiway(game_config) = config.game else {
...
type AutoPreviewReply = (u64, Result<app_core::auto_run::AutoRunDerivation, String>);

/// Debounced, background-threaded `app_core::auto_run::derive_auto_run` preview
...
impl AutoPreviewState {
    ...
    fn dispatch(&mut self, model: &Model, threads: usize, memory_budget_bytes: u64, hash: u64) {
        ...
        std::thread::spawn(move || {
            let result =
                app_core::auto_run::derive_auto_run(&game_config, threads, memory_budget_bytes)
