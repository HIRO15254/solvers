...
/// see `app_core::node_eval`'s doc comment -- so it must never block the UI
...
impl ResultsState {
    ...
    fn seat_range(&self, actor: u8) -> Option<Range> {
        let config: app_core::config::SolveConfig =
            toml::from_str(&self.solution.config_toml).ok()?;
        let app_core::config::GameSection::PreflopMultiway(game) = config.game else {
            return None;
        };
    ...
    fn start_eval(&mut self) {
        ...
        std::thread::spawn(move || {
            let result = app_core::node_eval::evaluate_mwsol_node_actions(
                &solution,
                &thread_path,
                samples,
                seed,
...
fn algorithm_seed(config_toml: &str) -> u64 {
    toml::from_str::<app_core::config::SolveConfig>(config_toml)
        .ok()
        .map(|config| match config.algorithm {
            app_core::config::AlgorithmSection::ExternalSamplingMccfr { seed, .. } => seed,
            _ => 0,
        })
        .unwrap_or(0)
}
...
fn seat_names_from_config(config_toml: &str) -> Vec<String> {
    let Ok(config) = toml::from_str::<app_core::config::SolveConfig>(config_toml) else {
        return Vec::new();
    };
    let app_core::config::GameSection::PreflopMultiway(game) = config.game else {
        return Vec::new();
