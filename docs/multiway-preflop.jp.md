...
平均の方 — が、同じチェックポイントでの実測では last-iterate が評価
シード 2 種で一貫して平均よりタイトだった(maxDevUp 0.181/0.168bb vs
0.543/0.424bb、argmax purification 併用で 0.143/0.072bb と全計測中最タイト)。
校正済みの線形割引が regret をほぼ収束済みの直近プレイ支配にしている
ためと考えられる。この傾向が設定・ラン長を跨いで成立するなら、
`strategy_sum` 配列を丸ごと落とす last-iterate 出力モード = **arena
メモリ半減・同予算でバケット 2 倍**が正当化される(真の last-iterate
保証を持つ手法は MMD/QRE 系文献を参照)。当面、エクスポートは保証付きの
平均のままとし、これは診断機能に留める。
